...
| **B5** ([#331](https://github.com/TheTechNetwork/Fleetwright/issues/331)) | **`peek` is documented as "how you find out whether work is done" and is useless on a stopped session.** Which is every session a returning user has. | **medium** | **Fixed.** `/api/peek` on a known-but-stopped session names the resume rather than answering "not running"; an unknown name still gets told it is unknown, because offering a resume for a session that never existed is a remedy that cannot work. Pinned by `test/returning-user.test.js`. |
...
| **C4** ([#320](https://github.com/TheTechNetwork/Fleetwright/issues/320)) | **The credential countdown is only in `fleet_verify`.** "Signed in (11m left)" was decisive and reachable only from a tool called out of desperation. | **high** | **Fixed.** The countdown is in `describeHealth`, so it arrives with every `fleet_health` rather than only from `fleet_verify`. Pinned by `test/mcp-remote.test.js`. |
...
| **D2** ([#323](https://github.com/TheTechNetwork/Fleetwright/issues/323)) | **A drifted host cannot be fixed from the product.** True, and for a different reason than recorded here: `update` is not denied — it came off `DEFAULT_DENY` — it is REFUSED, by the same version check as everything else, before the verb is read. The drift error also named no remedy at all; the one that names `agent-hub update --restart` is `unknown_verb`, which is a different failure and IS fixable from the fleet. Now explained AND broken: `unsupported_version` says which version each side speaks, and `update` travels in a frozen envelope whose version is not checked — the coordinator labels it with the version a drifted host reports, so a box that is BEHIND is repaired from the app. Two cases are left, and the message names both rather than promising: a host AHEAD of the coordinator is waiting for a deploy, and a box pinned to a channel with no newer release still needs the installer run on the machine. | **high** |
...
| **G4** ([#335](https://github.com/TheTechNetwork/Fleetwright/issues/335)) | **`agent-hub.md` is upstream's README**, with upstream's install steps and version claims, banner'd only on the install section. | **low** | **Fixed.** The upstream *Install* section is removed rather than banner'd — it cloned `ambersecurityinc/agent-hub` and three of its four steps configured a Telegram bot. Its Security section also still named `AGENT_HUB_TELEGRAM_ALLOWED_USERS` as the root allowlist, which was the wrong access control in the one section where that matters most; it now names the hub API token. The adapter itself has since been archived ([`telegram.md`](./telegram.md)), so that variable is read by nothing at all. |

## H. Documents disagree about what is real

| # | Finding | Severity |
|---|---|---|
| **H1** ([#336](https://github.com/TheTechNetwork/Fleetwright/issues/336)) | **Four documents, four app maturity levels.** README links live TestFlight/Play; ROADMAP section 3 says shipped; `deployment.md` says "no notification has been delivered to a real phone, and no app has been run by a person"; `app-testing.md` says both apps have been run in simulators. | **high** | **Fixed.** One table — `app-parity.md` §"What is actually proven about the apps" — with README and `deployment.md` deferring to it by anchor. |
