...
| **G4** | **still open, and sharper.** SEC-SESSION-5 holds: the fleet deploys rootless, and nothing yet asserts that uid 0 in a container maps to the unprivileged service uid on the host. Checked against podman's own reference in [`recommendations-review.md`](./recommendations-review.md) §1: `sandboxArgv` passes no `--userns`, and rootless podman's default for that is `host` inside the caller's namespace, so container root IS the service uid today — the account that owns the credential and the socket directory. The fix is `--userns=nomap` on every container that touches a session volume, and the test is `/proc/self/uid_map` inside a session. (This row briefly claimed "fixed" on the strength of the pane-detection work in `test/real-panes.test.js`, which belongs to a different finding entirely — recorded rather than erased, because a security row marked fixed by an unrelated bug fix is exactly the failure this register exists to catch.) |
