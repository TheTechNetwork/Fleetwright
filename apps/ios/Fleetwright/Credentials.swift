...
struct CredentialsView: View {
    ...
    /// WHICH BOX THE FLOW IN FRONT OF YOU IS ON.
    ///
    /// The footer used to say `host ?? "that machine"`, which was written to be
    /// honest about not knowing — and not knowing was itself the bug. A Claude
    /// sign-in happens in a pane on ONE machine and the code goes back to that
    /// same pane, so a flow that cannot name its box is a flow nobody can
    /// finish.
    @State private var pendingHost: String?
    /// A sign-in with more than one box to choose from. Asked, never guessed:
    /// starting a login pane on a machine somebody did not pick means the code
    /// they paste goes somewhere they did not intend.
    @State private var choosingHost: Fleet.Connections.Available?
    ...
    var body: some View {
            ...
            if let pending {
                Section {
                    ...
                    Text(pending.isSignIn
                         ? "This page was generated by \(pendingHost ?? host ?? "that machine") for this attempt, and the code goes back to the same box."
        ...
        .confirmationDialog(
            "Which machine?",
            isPresented: Binding(get: { choosingHost != nil }, set: { if !$0 { choosingHost = nil } }),
            titleVisibility: .visible,
        ) {
            if let provider = choosingHost {
                ForEach(hostsToOffer(provider), id: \.self) { candidate in
                    Button(candidate) {
                        choosingHost = nil
                        Task { await begin(provider, on: candidate) }
                    }
                }
            }
            Button("Cancel", role: .cancel) { choosingHost = nil }
        } message: {
            // Said, because the consequence is not obvious: this is not "where
            // is it stored", it is "where does the browser page come from and
            // where does the code go back to".
            Text("The sign-in page is generated by one machine and the code goes back to that same one.")
        }
        .task {
    ...
    private func targetHost(_ provider: Fleet.Connections.Available) -> String? {
        ...
        // ONE BOX IS NOT A CHOICE. With a single machine there is nothing to
        // ask about and a picker would be furniture.
        let known = connections.hosts ?? []
        if known.count == 1 { return known.first }
        // More than one, or none known: the confirmation dialog is the answer.
        // This deliberately no longer falls back to `.first`.
        return nil
    }

    /// Which machine a sign-in should go to, having asked.
    ///
    /// IT USED TO GUESS, and the guess reached a person: `absentFrom?.first ??
    /// hosts?.first`, which starts a login pane on whichever box the registry
    /// happened to list first. When even that came back nil the request went
    /// out with no host at all, and the coordinator answered "Which box?
    /// deb13-staging, deb132." — a correct refusal, rendered as a detached card
    /// under a flow that looked started. Screenshotted on a two-host fleet.
    ///
    /// Neither half was right. Guessing sends the code somebody pastes to a
    /// machine they did not choose; sending nothing produces an answer the
    /// screen has no way to act on. Asking is the third option and it is the
    /// one the rest of this project already takes when a host is ambiguous.
    private func hostsToOffer(_ provider: Fleet.Connections.Available) -> [String] {
        provider.isSignIn ? (connections.hosts ?? []) : []
    }
    ...
    private func begin(_ provider: Fleet.Connections.Available, on target: String? = nil) async {
        ...
        defer { busy = false }
        do {
            let chosen = target ?? host
            let reply = try await Fleet(settings: settings).connect(host: chosen, provider: provider.provider)
            if let fresh = reply.connections {
                connections = fresh
                pending = fresh.catalogue.first { $0.provider == provider.provider }
                pendingHost = chosen
            }
            // A REFUSED CONNECT LEAVES NO FLOW ON SCREEN. There is no pane
            // waiting, no URL, and nothing a code could be pasted into — a
            // three-step form under a refusal is an invitation to finish
    ...
    private func link(_ provider: Fleet.Connections.Available) async {
        ...
        do {
            ...
            //
            // `pendingHost` FIRST, and this was the same bug as the one above
            // wearing different clothes: on the fleet-wide screen `host` is
            // nil, so step 3 sent the code with no host and the coordinator
            // refused it with "Which box?" — after the person had already left
            // the app, signed in, and come back with a live credential in the
            // clipboard. The pane waiting for that code is on exactly one
            // machine and `begin` knows which.
            let reply = provider.isSignIn
                ? try await Fleet(settings: settings).link(host: pendingHost ?? host, provider: provider.provider, secret: sending)
    ...
    private func clear() {
        pending = nil
        // Cleared with the flow it belongs to. A stale host outliving its pane
        // would send the next code to a machine that stopped waiting.
        pendingHost = nil
        secret = ""
    }
}
