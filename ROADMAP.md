...
| Session detail screen; notification actions | **partial** | answering works in-app, and a tapped notification now lands on the session list rather than whatever tab was showing last; the notification *action* (answer without opening the app) is still to build |
