...
| **A push relay for other people's coordinators** — they post "wake this device", we forward it with our APNs key and FCM service account | **wanted** ([#348](https://github.com/TheTechNetwork/Fleetwright/issues/348)) | The one thing that CANNOT be self-hosted. A device token is issued for a specific app, so `push.js` sends to `apns-topic: network.thetech.fleetwright` with a key from our Apple team — somebody else's coordinator cannot wake our app on their phone, and their only alternative is shipping their own apps at $99/yr plus review, to change one hostname. **It carries a WAKE, NOT A MESSAGE**, and that is the design rather than a nicety: text would mean seeing session names and prompts, and the sentence on /docs that says there is no server of ours in the middle would stop being true. The app fetches the detail from its OWN coordinator. `psychology.md` already argues the important state is "nothing needs you" — a wake is that state changing. **Open question worth settling on hardware first:** whether a contentless notification is good enough to act on, given app-parity.md records that push has never been confirmed on a physical device at all |
