...
| **Package the host** — versioned tarball + manifest instead of a monorepo checkout | **partial** | `docs/packaging.md`. Path-scoped "behind" **done**; `host-release.yml` publishes the tarball and manifest on release, and `/update` consumes the manifest with a git fallback — both **done**. `/update` on a packaged box now also re-runs the release's own installer with `--repair`, so units, hook, sudoers and the helper follow the release (docs/packaging.md, "Root's half follows the release"). Next: the installer's *first* install fetching a release instead of a clone (step 4 there) |
