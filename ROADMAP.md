...
| Read logs from the app | **partial** | This row said **done** for a week and it was not: `Fleet.swift` and `Fleet.kt` both implement the `logs` call and no screen on either phone calls it. The verb is real on the host, the CLI and the MCP server (`fleet_logs`); the phones need a screen. Corrected by the audit that found it, not by shipping the screen |
...
| Runner tokens from the app | wanted | minted and revoked with curl today; `docs/runner-central.md` shows the call. Neither phone has a screen, and the parity test records that as a gap rather than a decision |
