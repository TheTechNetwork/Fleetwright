...
| feature | status | where |
|---|---|---|
| **Secret references** — `start --secret <name>` names a secret the host holds; the coordinator learns only the name | **done** (protocol v4) | `docs/trust.md` step 2, `docs/credential-broker.md`. The store is `AGENT_HUB_SECRETS_DIR`, one file per name; the session fetches the value at runtime with `fleet-secret <name>` over the per-session socket, scoped to what it was granted, logged by name and never by value — so nothing is seeded into the container and a rotation reaches a running session. A `secrets` verb lists the names so the phones offer a picker, the way `profiles` does. It is the first step of the proxy work: it bounds what the COORDINATOR learns, and does nothing yet about what the SESSION does with the value — that is the proxy below |
...
| **Cloudflare OAuth app instead of a pasted API token** — Authorization Code, scopes chosen at client registration | **done** ([#347](https://github.com/TheTechNetwork/Fleetwright/issues/347)) | `docs/connectors.md`. The GitHub shape transferred whole: its own callback (`/oauth/cloudflare/callback`), a single-use state binding host and person, the access token stored by `link`, the refresh token deposited by `renew`, and the client secret on the config frame so the sidecar renews on its timer. Three values turn it on — client id, client secret, and the client's registered scope list (`AGENT_FLEET_CLOUDFLARE_SCOPES`, dot-form names plus `offline_access`); absent any of them the paste route stays, first-class. Client secret rather than PKCE, which is Cloudflare's own guidance for a server-side client — the coordinator is confidential and holds the secret anyway. What was predicted held: a PRIVATE client serves the operator today; a guest bringing their own Cloudflare account still needs the client made PUBLIC, which requires domain verification and is IRREVERSIBLE, so that switch stays a deliberate decision. **What it does not change:** the token still lands on the host as a real credential — a better way to MINT one, not the credential-terminating proxy in `docs/trust.md` |
...
| Update / upgrade / reboot a box from the app | **done** | both apps; reboot keeps the pin-plus-typed-hostname guard |
| Read logs from the app | **done** | This row said **done** for a week before it was: the client call existed on both phones and no screen made it. Now the host page on each phone offers one button per journal the box says it can read (`logs` on health), and every session has Output — what it printed, which outlives the pane |
| Start a temporary runner from the phone (`provision`) | **done** | Under Add a machine on iOS and Hosts on Android: a platform, the minutes, one button — drawn only when the fleet snapshot names a runner repository, so the control exists on exactly the fleets where it does something. `fleet_provision` from the MCP server, as before |
| Runner tokens from the app | **done** | Runner tokens under Add a machine on iOS and Hosts on Android: mint one for a repository, see it once, see whose each is and whether it has ever been used, revoke. The parity test no longer carries the two routes as a gap |
| Session detail screen; notification actions | **done** | The notification action is **done**: a session asking something the fleet has words for arrives with two buttons on it, and pressing one answers without the app coming forward. The words are the fleet's rather than the CLI's, because an iOS action title is fixed at category registration — so `prompt.js` writes the answers beside the questions it already writes, and the host resolves which digit each button types against the pane it actually drew. It survives `AGENT_FLEET_PROMPT_TEXT=0`, which the labels themselves do not. `sentAt` is finally read: an hour after it was sent, the same hour `PUSH_TTL_S` would have delivered it for, a press opens the session with a notice instead of answering. Android gives up its tray delivery to get the buttons, so an answerable notification does not reach a force-stopped app. The **session detail screen** is built, on both phones: tap a session's title and the page (iOS) or sheet (Android) leads with the state sentence in the title size, then the question and its answers or Remote Control front and centre when there is no prompt, then the pane read through `peek` and watched on a schedule that ends (ten looks three seconds apart, nine ten seconds apart, then a button), then the same actions with room. The state vocabulary is owned once on each model as `stateSentence`, and every sentence is a fact the frame carries: "At its prompt", never "finished", because a session between two steps looks the same. `test/session-screen.test.js` holds the two phones to the same words, schedule and order |
...
| **A design system, shared with the console** | **partial** | `docs/design-system.md`. One token table in `test/fixtures/parity/design-tokens.json`, read by `console.css`, `Design.swift` and `Design.kt` and held equal by `test/design-parity.test.js`. Landed on the session list and its cards on both phones, and on the theme everything else hangs off — Android's dynamic (wallpaper) colour is gone, which is what made "amber means waiting for you" untrue on a device. The settings, credentials, start, files and bin screens are still Material's and SwiftUI's layouts on the right palette |
...
| Browser control surface / PWA | **credentials half done**, the rest wanted | `/me` is an installable page that signs a member in and adds their own Claude, GitHub and Cloudflare accounts — `src/fleet/coordinator/member-page.js`, served by both coordinators. It was not a convenience: an invited person with no phone could sign in and then do nothing, because `connect`/`link` are withheld from agents (`DEFAULT_DENY`) and a session whose actor has no linked Claude account is refused, so the app was the only surface that could finish an onboarding the app was not required for. Driving sessions from a browser is still wanted and `/me` deliberately does none of it: a second place to start and stop work is a second place for the two to disagree about what is running |
...
| **WhatsApp as a chat surface** — third-party agents | **designed, not built** | [`docs/whatsapp.md`](./docs/whatsapp.md). WhatsApp is rolling out chats with third-party agents: the person mints an API key in their own settings and pastes it into whatever hosts the agent, five per account, one-to-one only, and **not end-to-end encrypted**. The design answers the question that archived Telegram and closed Telnyx — it does not authenticate the WhatsApp user at all. A bridge is **another MCP client**, holding one `fwk_…` minted in the app, and `/mcp` takes it as a bearer with no OAuth dance, so nothing in this repository changes and there is no `whatsapp:<number>` actor. It lives in its own repository for the same reason Telnyx does. Three bounds carry over rather than being promised: no free text (structural — no verb carries any), `answer` stays in `DEFAULT_DENY` because the person is right there and the model must still not choose for them, and it is a **pull** surface, since push already spends the interruption budget. The cost is the encryption: a Remote Control URL may never cross it. **Blocked on availability** — Android beta, a few countries — and on reading the key-minting flow from the real settings screen, which is the one fact the whole design rests on |
...
| **Package the host** — versioned tarball + manifest instead of a monorepo checkout | **partial** | `docs/packaging.md`. Path-scoped "behind" **done**; `host-release.yml` publishes the tarball and manifest on release, and `/update` consumes the manifest with a git fallback — both **done**. `/update` on a packaged box now also re-runs the release's own installer with `--repair`, so units, hook, sudoers and the helper follow the release (docs/packaging.md, "Root's half follows the release"). The *first* install fetches a release too: the one-liner checks the tarball against the manifest before unpacking and hands over to the installer inside, so a fresh box never needs git (step 4 there, **done**). What is left is step 5, dropping the git path, and it is kept on purpose for `--from-source` and for repositories off GitHub |
...
| Scheduled credential refresh on the host itself, so a box is never signed out at the moment somebody starts a session | **done** | `src/core/keepalive.js`, run by agent-hub hourly and once at startup (`AGENT_HUB_CREDENTIAL_KEEPALIVE_MS`), for the shared credential and every linked account. A ladder, cheapest rung first: `claude auth status`, then a one-shot prompt that costs a few tokens and unambiguously exercises the credential. The verdict is read off the credential file, whether the expiry MOVED, never off an exit code. Provider tokens renew by a different mechanism, an exchange the sidecar runs hourly with the secret the coordinator supplies |
| **Per-session telemetry** — what a session cost, how long it worked, and whether it is blocked on a person | **wanted** | [`docs/telemetry.md`](docs/telemetry.md). The first answer to this was that the data is not available, and that is half wrong. The four org APIs (Claude Code Analytics, Usage & Cost, Enterprise Analytics, Compliance) are gated on organisation type rather than on permission, so a fleet signed in with a personal Pro or Max account cannot mint any of their keys. But **OpenTelemetry is not gated and is not a channel to Anthropic** — `OTEL_EXPORTER_OTLP_ENDPOINT` is our address, and it is a finer grain than the API sells: 8 metrics, 13 event types, and `tool.blocked_on_user`, which is the first machine-readable answer to the thing `docs/mcp.md` ends on — a finished session looks exactly like an idle one. **The price is named before the feature:** `DISABLE_TELEMETRY` and `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` also disable the feature-flag evaluation Remote Control depends on, so the off switch costs the handoff this fleet is built around; default-deny egress at the container keeps both and replaces a claim with a log, which is `docs/trust.md`'s argument already. And the privacy text is rewritten BEFORE anything is collected, not after — today's promises cover the app and the relay, not sessions, which is exactly the gap that reads as misleading later. The cheapest next step needs no code: `AGENT_HUB_SANDBOX_ARGS` already carries `--env` into `podman run` |
| Node-coordinator socket-leg parity tests (the ws analogue of the executed OpenAPI spec) | **done** | `worker/test/parity.test.js` drives ONE host client — `identity.js` and `ws.js`, the code a real sidecar ships — through both the Worker in workerd and a Node coordinator in process, and asserts the same result on each: a health frame lands the host on `healthy`, an intent round-trips with the hostId attached and no host param on the wire, a garbage frame does not drop the socket, an unenrolled host is refused the upgrade, and `/healthz` reports the running protocol. The host client being the same object for both halves is the contract: if a real sidecar can drive both, the two are the transport swap the design claims |
