...
import { mkdtempSync, mkdirSync, writeFileSync, readFileSync, readlinkSync, existsSync, rmSync, symlinkSync, renameSync, readdirSync, chmodSync } from 'node:fs';
...
test('a release it cannot delete does not fail an update that already swapped', { skip: asRoot && 'root ignores mode bits' }, async () => {
  ...
  // `update failed: EACCES …/releases/main-81`. It must be logged and stepped
  // over: the update stays ok, and every OTHER old release is still pruned.
  const box = makeBox('v1');
  for (let i = 0; i < 15; i++) mkdirSync(path.join(box.base, 'releases', `old-${i}`), { recursive: true });
  ...
  // The stuck release is left behind, not silently claimed removed.
  assert.equal(existsSync(path.join(box.base, 'releases', 'old-0')), true);
